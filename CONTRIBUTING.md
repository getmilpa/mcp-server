# Contributing to Milpa McpServer

Thanks for your interest in contributing! Milpa McpServer is the Model Context Protocol
(MCP) transport core of the Milpa framework — a transport-agnostic JSON-RPC 2.0 dispatcher
(`initialize`, `notifications/initialized`, `tools/list`, `tools/call`) over the
`milpa/tool-runtime` registry, plus the auth seam (`TokenValidatorInterface` / `Principal`)
a host implements to turn a bearer token into an authenticated caller. It builds on
`milpa/core` and `milpa/tool-runtime`, with no product coupling — no HTTP kernel, no
Doctrine, no concrete token store.
...
- **Respect the tier boundary.** Milpa McpServer depends on `milpa/core` and
  `milpa/tool-runtime`, never the reverse. Do not introduce a dependency on Doctrine, a
  concrete HTTP kernel, a concrete container, or any product/plugin code — the concrete
  transport (HTTP, SSE, stdio) and the concrete `TokenValidatorInterface` implementation
  belong to the host application, not to this package.
...
The whole Milpa family (`milpa/core`, `milpa/http`, `milpa/tool-runtime`, `milpa/mcp-server`)
